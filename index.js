...
function replaceCurrencyCode(courseData) {
  ...
  .then((data) => {
    ...
    courseData.forEach((currentCurrency) => {
      ...
      currentCurrency.currencyCodeB = mapWithCodes.get(currentCurrency.currencyCodeB);
    })
...
function showCourse(courseData) {
  courseData.map((item) => {
    if(item.rateBuy && item.rateSell) {
      ...
      div.append(`
      Монобанк продает ${item.currencyCodeA} за ${item.rateSell} ${item.currencyCodeB}, а покупает за ${item.rateBuy} ${item.currencyCodeB}
      `);
